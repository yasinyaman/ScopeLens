...
def test_logged_in_viewer_cannot_approve(anon_client: TestClient, app_context: AppContext) -> None:
    # The viewer is granted the demo project (A3 isolation) — the test measures PMO-approval denial.
    app_context.user_store.create("viewer1", "p-123", "viewer", projects=["demo"])
    anon_client.post("/login", data={"username": "viewer1", "password": "p-123"})
